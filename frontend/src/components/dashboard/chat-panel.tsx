...
function ChatPanel({ sessionToken, userEmail, connections, footer }: Props) {
  ...
  const suggestions = userEmail
    ? [
        ...BASE_SUGGESTIONS,
        `Create a meeting tomorrow at 10am for 30 minutes with ${userEmail} as attendee`,
      ]
    : BASE_SUGGESTIONS;
      ...
      setMessages((current) =>
        current.map((message) =>
          message.id === assistantId
            ? {
                ...message,
                content: "Could not reach the agent API. Please try again.",
              }
            : message,
        ),
      );
